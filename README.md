...
opencode skills are markdown. They are loaded as system context for the agent. Putting the entire procedure in one file keeps the agent's prompt focused: it sees the strategy, the thirteen pitfalls, and the thirteen-item verification checklist in one place, with no extra navigation.
